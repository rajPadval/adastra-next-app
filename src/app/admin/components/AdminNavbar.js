import { setSideToggle, setTab } from "@/redux/slices/navSlice";
  ...
  return (
    <div
      className={`fixed overflow-hidden z-50 flex flex-col border-lg bg-gray-700 m-2 rounded-lg text-white w-[230px] h-[97vh] transition-transform ${
        !sideToggle ? `translate-x-0` : `-translate-x-[250px]`
      }`}
    >
      ...
      <div className="flex flex-col gap-5 py-3">
        <div className="flex flex-col gap-2 px-5 py-3 text-start list-none">
          <a
            className={`px-2 py-1 rounded-lg hover:bg-purple-400 font-bold transition-all cursor-pointer ${
              tab == "Events" ? "text-purple-400 scale-110" : "text-white"
            }`}
            onClick={() => dispatch(setTab("Events"))}
          >
            Events
          </a>
          <a
            className={`px-2 py-1 rounded-lg hover:bg-purple-400 font-bold transition-all cursor-pointer ${
              tab == "Messages" ? "text-purple-400 scale-110" : "text-white"
            }`}
            onClick={() => {
          ...
          <a
            className={`px-2 py-1 rounded-lg hover:bg-purple-400 font-bold transition-all cursor-pointer ${
              tab == "Blogs" ? "text-purple-400 scale-110" : "text-white"
            }`}
            onClick={() => dispatch(setTab("Blogs"))}
          >
            Blogs
          </a>
          <a
            className={`px-2 py-1 rounded-lg hover:bg-purple-400 font-bold transition-all cursor-pointer ${
              tab == "Members" ? "text-purple-400 scale-110" : "text-white"
            }`}
